...
while toc < 45%스탑워치가 작동한 이후 25초까지에 대해서만 실행한다.
...
Resp = zeros(1,16);% 수신 신호를 결정하기 위한 rPreamble에서의 수단이다. DFTF 과정 및 periodogram이 포함된다.
tt = 1:16000;% rPreamble의 길이. 이 길이에 대한 DTFT를 진행한다.
for i = 1:16% modulation이 16개의 bit씩 진행되었기 때문에 demodulation 또한 16개의 bit씩 진행하여 순차적으로 저장한다.
...
while(length(Data)>=4000)% tranmitter 부분에서 modulation 행렬곱 과정은 modulator의 높이에 따라 16개의 bit씩 진행되었다. 또한, modulator의 폭에 따라 4000개의 data가 생성되었다. 즉, demodulation은 4000개씩 진행하여 16개의 값을 반환한다.
    ...
    resp = zeros(1,16);% 수신 신호를 결정하기 위한 bits에서의 수단이다. DTFT 과정 및 periodogram이 포함된다.
    for k = 1:16% 4000개의 대상으로 demodulation을 진행하면 16개의 output이 생성된다. output은 DTFT 값의 제곱으로써 앞에서 구한 rPreamble의 DTFT 값과 비교 과정을 거치게 된다.
    ...
    bits = [bits resp>0.008];% threshold 넘으면 1, 아니면 0 // resp값을 0과 1로 binarize하여 순서대로 저장한다. 송신한 신호가 무엇인지 직접적으로 드러나는 부분이다.
end
